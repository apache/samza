...
/**
 * Factory methods for creating a {@link Window}. Provides APIs for specifying different types of {@link Window}s.
 * A {@link Window} can have a specified keyFunction that is used to group {@link MessageEnvelope}s in the {@link Window}.
 *
 * <p> The following types of {@link Window}s are supported.
 * <ul>
 *  <li>Tumbling Windows: A tumbling window defines a series of non-overlapping, fixed size, contiguous time intervals.
...
public final class Windows {
  ...
  /**
   * Creates a {@link Window} that groups incoming {@link MessageEnvelope}s using the provided keyFn into
  ...
  public static <M extends MessageEnvelope, K, WV> Window<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>>
    ...
    return new WindowInternal<M, K, WV>(defaultTrigger, aggregateFunction, keyFn, null);
  }
  ...
  public static <M extends MessageEnvelope, WV> Window<M, Void, WindowKey<Void>, WV, WindowOutput<WindowKey<Void>, WV>>
    ...
    return new WindowInternal<M, Void, WV>(defaultTrigger, aggregateFunction, null, null);
  }
   ...
   * @return the created {@link Window} function.
   */
  public static <M extends MessageEnvelope> Window<M, Void, WindowKey<Void>, Collection<M>, WindowOutput<WindowKey<Void>, Collection<M>>> tumblingWindow(Duration duration) {
    BiFunction<M, Collection<M>, Collection<M>> aggregator = (m, c) -> {
      c.add(m);
      return c;
    };
   ...
   * @return the created {@link Window} function.
   */
  public static <M extends MessageEnvelope, K, WV> Window<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>> keyedSessionWindow(Function<M, K> keyFn, Duration sessionGap, BiFunction<M, WV, WV> aggregateFunction) {
   ...
   * @return the created {@link Window} function.
   */
  public static <M extends MessageEnvelope, K> Window<M, K, WindowKey<K>, Collection<M>, WindowOutput<WindowKey<K>, Collection<M>>> keyedSessionWindow(Function<M, K> keyFn, Duration sessionGap) {
  ...
  public static <M extends MessageEnvelope, WV> Window<M, Void, WindowKey<Void>, WV, WindowOutput<WindowKey<Void>, WV>> globalWindow(BiFunction<M, WV, WV> aggregateFunction) {
    return new WindowInternal<M, Void, WV>(null, aggregateFunction, null, null);
  }
  ...
  public static <M extends MessageEnvelope, K, WV> Window<M, K, WindowKey<K>, WV, WindowOutput<WindowKey<K>, WV>> keyedGlobalWindow(Function<M, K> keyFn, BiFunction<M, WV, WV> aggregateFunction) {
    return new WindowInternal<M, K, WV>(null, aggregateFunction, keyFn, null);
  }
