...
public class TestStreamApplicationDescriptorImpl {
  ...
  public void testGetNextOpIdIncrementsId() {
    ...
    assertEquals("appName-1234-merge-0", streamAppDesc.getNextOpId(null, OpCode.MERGE, null));
    assertEquals("appName-1234-join-customName", streamAppDesc.getNextOpId(null, OpCode.JOIN, "customName"));
    assertEquals("appName-1234-map-2", streamAppDesc.getNextOpId(null, OpCode.MAP, null));
  }
  ...
  public void testGetNextOpIdRejectsDuplicates() {
    ...
    StreamApplicationDescriptorImpl streamAppDesc = new StreamApplicationDescriptorImpl(appDesc -> { }, config);
    assertEquals("appName-1234-join-customName", streamAppDesc.getNextOpId(null, OpCode.JOIN, "customName"));
    streamAppDesc.getNextOpId(null, OpCode.JOIN, "customName"); // should throw
  }
  ...
  public void testOpIdValidation() {
    ...
    try {
      streamAppDesc.getNextOpId(null, OpCode.FILTER, null);
      streamAppDesc.getNextOpId("", OpCode.FILTER, "");
      streamAppDesc.getNextOpId(" ", OpCode.FILTER, " ");
      streamAppDesc.getNextOpId("\t", OpCode.FILTER, "\t");
    } catch (SamzaException e) {
      fail("Received an error with a null or empty description and user defined ID.");
    }

    List<String> validUserDefinedIds = ImmutableList.of("op_id", "op-id", "1000", "op_1", "OP_ID");
    for (String validUserDefinedId: validUserDefinedIds) {
      try {
        streamAppDesc.getNextOpId(null, OpCode.FILTER, validUserDefinedId);
      } catch (Exception e) {
        fail("Received an exception with a valid user defined ID: " + validUserDefinedId);
      }
    }

    List<String> validDescriptions = ImmutableList.of("desc_1", "desc-2", "1000", "op_1", "DESC_1");
    for (String validDescription: validDescriptions) {
      try {
        streamAppDesc.getNextOpId(validDescription, OpCode.FILTER, null);
      } catch (Exception e) {
        fail("Received an exception with a valid description: " + validDescription);
      }
    }

    List<String> invalidUserDefinedIds = ImmutableList.of("op id", "op#id");
    for (String invalidUserDefinedId: invalidUserDefinedIds) {
      try {
        streamAppDesc.getNextOpId(null, OpCode.FILTER, invalidUserDefinedId);
        fail("Did not receive an exception with an invalid operator ID: " + invalidUserDefinedId);
      } catch (SamzaException e) { }
    }

    List<String> invalidDescriptions = ImmutableList.of("desc 1", "desc#1");
    for (String invalidDescription: invalidDescriptions) {
      try {
        streamAppDesc.getNextOpId(invalidDescription, OpCode.FILTER, null);
        fail("Did not receive an exception with an invalid description: " + invalidDescription);
      } catch (SamzaException e) { }
    }
  }
