...
public class WindowExample implements StreamApplication {
  ...
  public void init(StreamGraph graph, Config config) {
    ...
    MessageStream<PageViewEvent> inputStream =
        graph.getInputStream("inputStream", new StringSerde("UTF-8"), new JsonSerde<PageViewEvent>(), (k, m) -> m);
    OutputStream<String, Integer, WindowPane<Void, Integer>> outputStream =
        graph.getOutputStream("outputStream", new StringSerde("UTF-8"), new IntegerSerde(),
            m -> m.getKey().getPaneId(), m -> m.getMessage());
