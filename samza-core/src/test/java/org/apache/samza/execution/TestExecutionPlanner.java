...
public class TestExecutionPlanner {
  ...
  public void setup() {
    ...
    system1Map.put("input4", IntermediateStreamPartitionPlanner.MAX_INFERRED_PARTITIONS * 2);
  ...
  public void testCreateProcessorGraph() {
    ...
    JobGraph jobGraph = planner.createJobGraph(graphSpec.getConfig(), new JobGraphJsonGenerator(graphSpec),
        new JobGraphConfigureGenerator(graphSpec), false);
  ...
  public void testCalculateJoinInputPartitions() {
    ...
    ExecutionPlanner.updateExistingPartitions(jobGraph, streamManager);
    new IntermediateStreamPartitionPlanner(config, graphSpec).calculateJoinInputPartitions(jobGraph);
  ...
  public void testMaxPartition() {
    ...
    assertEquals(32, IntermediateStreamPartitionPlanner.maxPartition(edges));

    edges = Collections.emptyList();
    assertEquals(StreamEdge.PARTITIONS_UNKNOWN, IntermediateStreamPartitionPlanner.maxPartition(edges));
  }

  @Test
  public void testMaxPartitionLimit() throws Exception {
    int partitionLimit = IntermediateStreamPartitionPlanner.MAX_INFERRED_PARTITIONS;
  ...
  public void testCreateJobGraphForTaskApplication() {
    ...
    ExecutionPlanner planner = new ExecutionPlanner(config, streamManager);
    JobGraph jobGraph = planner.createJobGraph(config, new JobGraphJsonGenerator(taskAppDesc),
        new JobGraphConfigureGenerator(taskAppDesc), false);
  ...
  public void testCreateJobGraphForLegacyTaskApplication() {
    ...
    ExecutionPlanner planner = new ExecutionPlanner(config, streamManager);
    JobGraph jobGraph = planner.createJobGraph(config, new JobGraphJsonGenerator(taskAppDesc),
        new JobGraphConfigureGenerator(taskAppDesc), true);
    assertEquals(1, jobGraph.getJobNodes().size());
    JobNode jobNode = jobGraph.getJobNodes().get(0);
    assertEquals("test-app", jobNode.getJobName());
    assertEquals("test-app-1", jobNode.getId());
