...
public class TestFutureUtil {
  ...
  public void testFutureOfMapCompletesSuccessfullyIfOnlyIgnoredErrors() {
    ...
    CompletableFuture<Map<String, String>> result = FutureUtil.toFutureOfMap(
        t -> FutureUtil.unwrapExceptions(CompletionException.class, t) instanceof SamzaException, map);
    assertTrue(result.isDone());
    result.join();
    assertFalse(result.isCompletedExceptionally());
