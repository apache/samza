...
class TaskStorageManagerBuilder extends MockitoSugar {
  ...
  def build: TaskStorageManager = {
    ...
    taskStores.keySet.foreach(storeName => {
      when(containerStorageManager.getStore(taskName, storeName)).thenReturn(JavaOptionals.toRichOption(taskStores.get("store1")).toOptional)
    });

    when(containerStorageManager.getAllStores(taskName)).thenReturn(taskStores.asJava)
