...
object ShellCommandConfig {
   ...
   *
   * The launch process for the application master depends on the value of this, since it needs to be known if the
   * application master should be launched in an isolated mode. This needs to be an environment variable, because the
   * value needs to be known before the full configs can be read from the metadata store (full configs are only read
   * after launch is complete).
   */
  val ENV_APPLICATION_MASTER_ISOLATION_ENABLED = "APPLICATION_MASTER_ISOLATION_ENABLED"

  /**
   * When running the application master in an isolated mode, it uses JARs and resources from a lib directory which is
   * provided by the framework. In some cases, it is necessary to use some resources specified by the application as
   * well. This environment variable can be set to a directory which is different from the framework lib directory in
   * order to tell Samza where application resources live.
   * This is an environment variable because it is needed in order to launch the application master Java process, which
   * means access to full configs is not available yet.
   * For example, this is used to set a system property for the location of an application-specified log4j configuration
   * file when launching the application master Java process.
   */
