...
public class MessageStreamImpl<M> implements MessageStream<M> {
  ...
  /** {@inheritDoc} */
  @Override
  public <TM> MessageStream<TM> map(String desc, MapFunction<? super M, ? extends TM> mapFn) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.MAP);
  ...
  /** {@inheritDoc} */
  @Override
  public MessageStream<M> filter(String desc, FilterFunction<? super M> filterFn) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.FILTER);
  ...
  /** {@inheritDoc} */
  @Override
  public <TM> MessageStream<TM> flatMap(String desc, FlatMapFunction<? super M, ? extends TM> flatMapFn) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.FLAT_MAP);
  ...
  /** {@inheritDoc} */
  @Override
  public <OM> MessageStream<OM> flatMapAsync(String desc, AsyncFlatMapFunction<? super M, ? extends OM> flatMapFn) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.ASYNC_FLAT_MAP);
  ...
  /** {@inheritDoc} */
  @Override
  public void sink(String desc, SinkFunction<? super M> sinkFn) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.SINK);
  ...
  /** {@inheritDoc} */
  @Override
  public MessageStream<M> sendTo(String desc, OutputStream<M> outputStream) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.SEND_TO);
  ...
  /** {@inheritDoc} */
  @Override
  public <K, WV> MessageStream<WindowPane<K, WV>> window(String desc, Window<M, K, WV> window, String userDefinedId) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.WINDOW, userDefinedId);
  ...
  public <K, OM, JM> MessageStream<JM> join(
      ...
      Serde<K> keySerde, Serde<M> messageSerde,
    ...
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.JOIN, userDefinedId);
  ...
  public <K, R extends KV<?, ?>, JM> MessageStream<JM> join(
      ...
      Object ... args) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.JOIN);
  ...
  /** {@inheritDoc} */
  @Override
  public MessageStream<M> merge(String desc, Collection<? extends MessageStream<? extends M>> otherStreams) {
    if (otherStreams.isEmpty()) return this;
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.MERGE);
  ...
  public <K, V> MessageStream<KV<K, V>> partitionBy(
      ...
      String userDefinedId) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.PARTITION_BY, userDefinedId);
  ...
  /** {@inheritDoc} */
  @Override
  public MessageStream<M> broadcast(String desc, Serde<M> serde, String userDefinedId) {
    String opId = this.streamAppDesc.getNextOpId(desc, OpCode.BROADCAST, userDefinedId);
