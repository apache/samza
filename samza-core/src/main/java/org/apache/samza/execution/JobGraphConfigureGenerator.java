...
public class JobGraphConfigureGenerator {
  ...
  private final StreamSerdeManager streamSerdeManager;
  ...
  JobGraphConfigureGenerator(ApplicationDescriptorImpl<? extends ApplicationDescriptor> appDesc) {
    this.streamSerdeManager = new StreamSerdeManager(appDesc);
  ...
  static Config mergeJobConfig(Config fullConfig, Config generatedConfig) {
  ...
  private void configureBroadcastInputs(Map<String, String> configs, Config config, List<String> broadcasts) {
  ...
  private JobConfig applyConfigureRewritersAndOverrides(Map<String, String> configs, Config config, JobNode jobNode) {
    String configPrefix = String.format(CONFIG_JOB_PREFIX, jobNode.getJobName());
  ...
  private void configureTables(Map<String, String> configs, Config config, List<TableSpec> tables, List<String> inputs) {
  ...
  private void configureSerdes(Map<String, String> configs, List<StreamEdge> inEdges, List<StreamEdge> outEdges,
    ...
    inEdges.forEach(edge -> {
      ...
      streamSerdeManager.addSerde(sid -> streamSerdeManager.getInputSerde(sid), streamId, streamKeySerdes, streamMsgSerdes);
    });
    outEdges.forEach(edge -> {
      String streamId = edge.getStreamSpec().getId();
      streamSerdeManager.addSerde(sid -> streamSerdeManager.getOutputSerde(sid), streamId, streamKeySerdes, streamMsgSerdes);
    });
  ...
  Collection<OperatorSpec> getReachableOperators(JobNode jobNode) {
    ...
    Set<OperatorSpec> inputOperatorsInJobNode = jobNode.getInEdges().stream()
        .filter(streamEdge -> streamSerdeManager.inputOperators.containsKey(streamEdge.getStreamSpec().getId()))
        .map(streamEdge -> streamSerdeManager.inputOperators.get(streamEdge.getStreamSpec().getId()))
        .collect(Collectors.toSet());
  ...
  private class StreamSerdeManager {
    ...
    private StreamSerdeManager(ApplicationDescriptorImpl<? extends ApplicationDescriptor> appDesc) {
