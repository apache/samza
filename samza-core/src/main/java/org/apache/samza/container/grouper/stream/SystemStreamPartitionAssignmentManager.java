...
public class SystemStreamPartitionAssignmentManager extends AbstractCoordinatorStreamManager {
   ...
   */
  public Map<SystemStreamPartition, String> readSSPTaskAssignment() {
    log.debug("Reading SystemStreamPartition assignment information");
    final Map<SystemStreamPartition, String> sspTaskAssignment = new HashMap<>();
    for (CoordinatorStreamMessage message: getBootstrappedStream(SetSSPTaskMapping.TYPE)) {
      ...
      sspTaskAssignment.put(Util.stringToSsp(setSSPTaskMapEntry.getKey()), setSSPTaskMapEntry.getTaskAssignment());
      log.debug("SystemStreamPartition: {} is mapped to task {}", setSSPTaskMapEntry.getKey(), setSSPTaskMapEntry.getTaskAssignment());
    }

    return sspTaskAssignment;
  }

  /**
   * Write the SystemStreamPartition to task mapping that is being maintained by this SystemStreamPartitionAssignmentManager
   * @param sspTaskAssignment The entries that needs to be written to the coordinator stream, the map takes the SSP
   *                          and it's corresponding taskName.
   */
  public void writeSSPTaskAssignment(Map<SystemStreamPartition, String> sspTaskAssignment) {
    for (Map.Entry<SystemStreamPartition, String> entry : sspTaskAssignment.entrySet()) {
