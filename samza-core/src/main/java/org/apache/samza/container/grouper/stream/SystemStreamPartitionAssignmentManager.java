...
public class SystemStreamPartitionAssignmentManager extends AbstractCoordinatorStreamManager {
   ...
   */
  public Map<SystemStreamPartition, String> readSystemStreamPartitionAssignment() {
    log.debug("Reading SystemStreamPartition assignment information");
    final Map<SystemStreamPartition, String> sspToTaskName = new HashMap<>();
    for (CoordinatorStreamMessage message: getBootstrappedStream(SetSSPTaskMapping.TYPE)) {
      ...
      sspToTaskName.put(Util.stringToSsp(setSSPTaskMapEntry.getKey()), setSSPTaskMapEntry.getTaskAssignment());
      log.debug("SystemStreamPartition: {} is mapped to task {}", setSSPTaskMapEntry.getKey(), setSSPTaskMapEntry.getTaskAssignment());
    }

    return sspToTaskName;
  }

  /**
   * Write the SystemStreamPartition to task mapping that is being maintained by this SystemStreamPartitionAssignmentManager
   * @param sspToTaskMapEntries The entries that needs to be written to the coordinator stream, the map takes the SSP
   *                          and it's corresponding taskName.
   */
  public void writeSSPTaskMapping(Map<SystemStreamPartition, String> sspToTaskMapEntries) {
    for (Map.Entry<SystemStreamPartition, String> entry : sspToTaskMapEntries.entrySet()) {
