...
public class SideInputStorageManager {
  ...
  public SideInputStorageManager(
    ...
    sspsToStore = new HashMap<>();

    storeToSSPs.forEach((storeName, ssps) -> {
      for (SystemStreamPartition ssp : ssps) {
        sspsToStore.put(ssp, storeName);
      }
    });
  }
  ...
  void flushOffsets() {
    storeToSSps.forEach((storeName, ssps) -> {
      ...
      try {
        String checkpoint = checkpointSerde.writeValueAsString(ssps);
  ...
  Map<SystemStreamPartition, String> loadStoreOffsets() {
    ...
    stores.keySet().forEach(storeName -> {
      ...
      if (storeLocation.exists() && !isStaleStore(storeLocation)) {
        try {
          File offsetFile = new File(storeLocation, OFFSET_FILE);
          String checkpoint = FileUtil.readWithChecksum(offsetFile);
  ...
  private void initializeStartingOffsets() {
    ...
    oldestOffsets.forEach((ssp, oldestOffset) -> {
      ...
      SystemAdmin admin = systemAdmins.getSystemAdmin(ssp.getSystem());
  ...
  private void initializeStoreDirectories() {
    ...
    stores.keySet().forEach(storeName -> {
      File storeLocation = getStoreLocation(storeName);
      if (isStaleStore(storeLocation) || !hasValidOffsetFile(new File(storeLocation, OFFSET_FILE))) {
  ...
  private boolean isStaleStore(File storeLocation) {
    ...
    if (storeLocation.exists()) {
      ...
      isStaleStore = (offsetFileLastModifiedTime != 0 && (clock.currentTimeMillis() - offsetFileLastModifiedTime) >= STORE_DELETE_RETENTION_MS);

      if (isStaleStore) {
        LOG.info(String.format("Store location: %s is stale since last modified time of offset file: %s, is older than "
            + "store deleteRetentionMs: %s.", storeLocation, offsetFileLastModifiedTime, STORE_DELETE_RETENTION_MS));
      }
    } else {
      LOG.info("Store location: " + storeLocation + " does not exist.");
    }
  ...
  /**
   * Checks if the store has a valid offset file. The criteria are as follows..
   *   1. existence of the offset file
   *   2. non-empty contents within the file.
   *
   * @param storeOffsetFile store offset file
   *
   * @return true if the offset file is valid, false otherwise
   */
  private boolean hasValidOffsetFile(File storeOffsetFile) {
    boolean hasValidOffset = false;
    if (storeOffsetFile.exists()) {
      String contents = FileUtil.readWithChecksum(storeOffsetFile);
      hasValidOffset = contents != null && !contents.isEmpty();
    }
