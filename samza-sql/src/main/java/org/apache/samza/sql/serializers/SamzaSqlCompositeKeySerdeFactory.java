...
public final class SamzaSqlCompositeKeySerdeFactory implements SerdeFactory<SamzaSqlCompositeKey> {
  public Serde<SamzaSqlCompositeKey> getSerde(String name, Config config) {
    return new SamzaSqlCompositeKeySerde();
  }

  public static final class SamzaSqlCompositeKeySerde implements Serde<SamzaSqlCompositeKey> {

    @Override
    public SamzaSqlCompositeKey fromBytes(byte[] bytes) {
      try {
        ObjectMapper mapper = new ObjectMapper();
        return mapper.readValue(new String(bytes, "UTF-8"), new TypeReference<SamzaSqlCompositeKey>() {
        });
      } catch (Exception e) {
    ...
    @Override
    public byte[] toBytes(SamzaSqlCompositeKey p) {
      try {
        ObjectMapper mapper = new ObjectMapper();
